...
**모든 UI는 반드시 모바일 반응형을 고려한다.** 모바일 퍼스트로 작성하고, `sm:` / `md:` / `lg:` 브레이크포인트로 확장한다. 모바일에서 깨지는 UI는 허용하지 않는다.
...
- 터치 타겟 최소 44px (`min-h-11`, `h-12` 등)
- 좌우 패딩 `px-4` 이상 (모바일에서 화면 가장자리에 붙지 않도록)
- 텍스트가 잘리지 않도록 `max-w` + `truncate` 또는 줄바꿈 허용
- `hidden sm:flex` 등으로 모바일에서 불필요한 요소 숨기기 가능
