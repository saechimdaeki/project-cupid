...
import { MatchHistoryListModal } from "./match-history-list-modal";

const RECENT_MATCH_LIMIT = 3;

type TimelinePanelContentProps = {
...
export function TimelinePanelContent({ events, onSelectEvent }: TimelinePanelContentProps) {
  ...
  const showViewAll = events.length > RECENT_MATCH_LIMIT;
          ...
          preview.map((event, index) => (
            ...
            >
              {index < preview.length - 1 ? (
                <span
                  aria-hidden
                  className="absolute bottom-[-1rem] left-[7px] top-6 w-px bg-gradient-to-b from-rose-200/80 to-orange-100/40"
                />
              ) : null}
              <span
                className={cn(
              ...
              <div className="rounded-2xl border border-rose-100/40 bg-rose-50/35 p-3.5">
                <div className="flex items-start justify-between gap-3">
                  <strong className="min-w-0 flex-1 break-words text-sm font-semibold leading-snug text-slate-800">
                    {event.title}
                  </strong>
                  <span className="shrink-0 pt-0.5 text-xs font-medium tabular-nums text-rose-400">
                    {event.happened_on}
                  </span>
                </div>
